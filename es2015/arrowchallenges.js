...
*/
var tripleAndFilter = arr => (arr.map(val => val * 3)).filter(val => val % 5 ===0);
...
//NEEDS WORK
var mapFilterAndReduce = arr => arr.map(val => val.firstName).filter(val => val.length < 5).reduce(acc, next => acc[next] = next.length);
